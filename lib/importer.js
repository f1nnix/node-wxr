(function() {
  ...
  normalizeDate = function(dateString) {
    ...
    console.log(date);
    return (date.getFullYear()) + "-" + (date.getMonth() < 10 ? '0' + String(date.getMonth()) : date.getMonth()) + "-" + (date.getDate()) + " " + (date.getHours()) + ":" + (date.getMinutes()) + ":" + (date.getSeconds());
  };
  ...
  module.exports = Importer = (function() {
    ...
    Importer.prototype.addPost = function(post) {
      var cat, catCDATA, category, contentEncoded, contentEncodedCDATA, creator, creatorCDATA, date, description, excerptEncoded, excerptEncodedCDATA, i, id, item, len, name, parent, ref, ref1, results, status, title, type;
      ...
      excerptEncodedCDATA = excerptEncoded.dat(post.excerptEncoded ? post.excerptEncoded : "This is the default excerpt for post. Please, provide your own.");
    ...
    Importer.prototype.addAttachment = function(options) {
      var creator, creatorCDATA, date, description, excerptEncoded, excerptEncodedCDATA, id, item, parent, status, title, type;
      if (options.attachmentURL) {
        item = this.channel.ele("item");
        id = item.ele("wp:post_id", {}, options.id ? options.id : Math.floor(Math.random() * 100000));
        ...
        creator = item.ele("dc:creator");
        creatorCDATA = creator.dat(options.author ? options.author : "admin");
